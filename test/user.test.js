...
redis_client_stub.hgetall.withArgs("bk-1234:misc").returns(bluebird.resolve(stub_user_misc));
...
describe("user", function() {
        ...
        describe("`created_at`", function() {
            ...
            it("returns `0` if it's a new user not yet saved", function() {
                const new_user = new User(
                    "bk-1234",
                    _.omit(stub_user_misc, ["created_at", "updated_at"])
                );
    ...
    describe("#save", function() {
        it("eventually set dirty to false", function() {
            return expect(this.user.save()).to.be.fulfilled.then(() => {
                expect(this.user.dirty).to.be.false;
                expect(redis_client_stub.hmset).to.have.been.calledOnce;
            });
        });

        describe("eventually save added source", function() {
            function _assert_added_source(user, source) {
                return expect(user.save()).to.be.fulfilled.then(() => {
                    expect(user.dirty).to.be.false;
                    expect(redis_client_stub.hmset).to.have.been.calledTwice;

                    const source_key = "bk-1234:sources";
                    const hmset_dict = { [source.id]: JSON.stringify(source) };
                    expect(redis_client_stub.hmset).to.have.been.calledWithExactly(
                        source_key,
                        hmset_dict
                    );
                });
            ...
            it("single source added multiple times (dedup)", function() {
                return bluebird
                    .all([this.user.add_source(this._source), this.user.add_source(this._source)])
                    .then(() => {
                        return _assert_added_source(this.user, this._source);
                    });
            });
        ...
        describe("eventually save deleted source", function() {
            function _assert_deleted_source(user, source) {
                return expect(user.save()).to.be.fulfilled.then(() => {
                    ...
                    expect(redis_client_stub.hdel).to.have.been.calledWithExactly(
                        source_key,
                        hdel_array
                    );
                });
        ...
        it("eventually populates `created_at` on first save", function() {
            ...
            return expect(new_user.save()).to.be.fulfilled.then(() => {
    ...
    describe("#load", function() {
        it("eventually returns `null` if user is not found", function() {
            return expect(User.load("user-not-found")).to.be.rejectedWith(
                errors.KinUnauthenticatedUser
            );
        });

        it("eventually returns a complete `User` if user is found", function() {
            return expect(User.load("bk-1234")).to.be.fulfilled.then(user => {
    ...
    describe("#create_alias", function() {
        it("should persist new alias", function() {
            ...
            expect(redis_client_stub.hset).to.have.been.calledWithExactly(
                "bk-1234:misc",
                "alias",
                "bk-5678"
            );
    ...
    describe("#add_source", function() {
        describe("with alias", function() {
            beforeEach(function() {
                redis_client_stub.hgetall.withArgs("alias-id:misc").returns(
                    bluebird.resolve({
                        alias: "aliased-id"
                    })
                );
            ...
            it("should throw `KinSourceAlreadyUsed` if the source is aliased to another user", function() {
                return expect(
                    this.user.add_source(
                        {
                            id: "alias-id"
                        },
                        true
                    )
                ).to.eventually.be.rejectedWith(errors.KinSourceAlreadyUsed);
            });
    ...
    describe("#get_alias", function() {
        beforeEach(function() {
            ...
            // B is a "real" user
            // C is available
            redis_client_stub.hgetall.withArgs("A:misc").returns(
                bluebird.resolve({
                    alias: "B"
                })
            );
            redis_client_stub.hgetall.withArgs("B:misc").returns(
                bluebird.resolve({
                    id: "B"
                })
            );
