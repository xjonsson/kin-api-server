...
describe('utlis', function () {
    ...
    describe('#ensured_logged_in', function () {
        ...
        it('load user when not yet authenticated and eventually calls `next` if successful', function () {
            ...
            return expect(utils.ensured_logged_in(req, {}, this.next_spy))
                .to.be.fulfilled
                .then(() => {
                    expect(req.user)
                        .to.deep.equal({
    ...
    describe('#validate_source', function () {
        it('return errors when source is not found', function () {
            ...
            expect(utils.validate_source(req, 'invalid_id'))
                .to.be.an.instanceof(errors.KinSourceNotFoundError);
        });
        ...
        it('returns error when source is disconnected', function () {
            ...
            expect(utils.validate_source(req, '1234'))
                .to.be.an.instanceof(errors.KinDisconnectedSourceError);
        });
    ...
    describe('#split_source_id', function () {
        it('returns an empty object if it\'s not a valid source id /.+-.+/', function () {
            expect(utils.split_source_id('kin1234'))
                .to.deep.equal({});
            expect(utils.split_source_id('kin-'))
                .to.deep.equal({});
            expect(utils.split_source_id('-1234'))
                .to.deep.equal({});
        });

        it('splits the source into a provider name and an ID', function () {
            expect(utils.split_source_id('kin-1234'))
                .to.deep.equal({
                    provider_name: 'kin',
        ...
        it('splits the source into a provider name and an ID at the first \'-\' encountered', function () {
            expect(utils.split_source_id('kin-1234-5678'))
                .to.deep.equal({
                    provider_name: 'kin',
