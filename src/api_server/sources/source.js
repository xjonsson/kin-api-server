...
function deauth_source(req, source) {
    ...
    logger.debug('%s removed source `%s` from user `%s`',
                 req.id, source.id, user.id);
}


function autoload_all_selected_layers(req, source) {
    const { provider_name } = split_source_id(source.id);
    return load_layers_mapping[provider_name](req, source)
        .then((source_layers) => {
            _(source_layers)
                .filter('selected')
                .forEach((layer) => {
...
function save_source(req, access_token, refresh_token, profile, done) {
    ...
    return bluebird
        .all(promises)
        .then(() => {
            logger.debug('%s added source `%s` to user `%s`',
                         req.id, source.id, user.id);
            done(null, user);
        })
        .catch((err) => {
