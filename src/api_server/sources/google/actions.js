...
const { logger, PROVIDER_NB_MONTHS_FUTURE, PROVIDER_NB_MONTHS_PAST, STATIC_HOSTNAME } = require('../../config');
...
function _format_event(layer_id, colors, event) {
    ...
    output.attendees = _.map(event.attendees, (attendee) => { // eslint-disable-line arrow-body-style
        return {
    ...
    output.reminders = _.map(_.get(event.reminders, 'overrides', {}), (reminder) => { // eslint-disable-line arrow-body-style
        return {
...
function _format_patch(event_patch) {
    const output = {};
    _(['start', 'end'])
        .filter(prop => !_.isEmpty(event_patch[prop]))
        .forEach((prop) => {
            output[prop] = {};
    ...
    if (!_.isUndefined(event_patch.attendees)) {
        output.attendees = _.map(event_patch.attendees, (attendee) => { // eslint-disable-line arrow-body-style
            return {
                email: attendee.email,
...
function _load_events_promise(req, source_id, uri, options, format_event_func, results) {
    return new GoogleRequest(req, source_id)
        .api(uri, options)
        .then((google_res) => {
            ...
            if (_.has(google_res, 'nextPageToken')) {
                ...
                return _load_events_promise(
                    req, source_id, uri, options, format_event_func, results);
            }
...
function delete_event(req, source, event_id) {
    ...
    return new GoogleRequest(req, source.id)
        .api(
            `calendars/${querystring.escape(short_layer_id)
                }/events/${querystring.escape(short_event_id)}`,
            { method: 'DELETE' }
        );
}
...
function load_contacts(req, source, query_input) {
    ...
    return new GoogleRequest(req, source.id, GCONTACTS_API_BASE_URL)
        ...
        .then((google_res) => {
            ...
            return parse_xml_string(google_res)
                .then((parsed_res) => {
