...
function _format_event(layer_id, event) {
    ...
    if (_.get(event, 'IsAllDay', false)) {
        ...
        output.start.date = moment
            ...
            .format(date_format);
        output.end.date = moment
            .tz(event.End.DateTime, event.End.Timezone)
            .format(date_format);
    } else {
        output.start.date_time = moment
            .tz(event.Start.DateTime, event.Start.Timezone)
            .format();
        output.end.date_time = moment
            .tz(event.End.DateTime, event.End.Timezone)
            .format();
    }
    ...
    output.attendees = _.map(_.get(event, 'Attendees', []), (attendee) => { // eslint-disable-line arrow-body-style
        return {
...
function _load_events_promise(req, source_id, uri, options, format_event_func, events = []) {
    return new OutlookRequest(req, source_id)
        .api(uri, options)
        .then((outlook_res) => {
...
function load_events(req, source, layer_id) {
    ...
    return _load_events_promise(
        ...
        options, _.partial(_format_event, layer_id)
    )
