...
router.patch(
    ...
    (req, res, next) => {
        ...
        patch_event_mapping[provider_name](req, source, event_id, req.body, notify_attendees)
            .then((event) => {
                logger.debug('%s updated event `%s` for user `%s`',
...
router.delete(
    ...
    (req, res, next) => {
        ...
        delete_event_mapping[provider_name](req, source, event_id)
            .then(() => {
                logger.debug('%s deleted event `%s` for user `%s`',
                             req.id, event_id, user.id);
                res.json(true);
                next();
            })
            .catch(next);
