...
const { load_contacts_mapping, load_layers_mapping, load_places_mapping } = require('../sources/actions_mappings');
...
router.get(
    ...
    (req, res, next) => {
        ...
        load_layers_mapping[provider_name](req, source)
            .then((layers) => {
                _.forEach(layers, (layer) => {
                    layer.selected = user.is_layer_selected(layer.id); // eslint-disable-line no-param-reassign
                });
                logger.debug('%s loaded `%d` layers in source `%s` for user `%s`',
                             req.id, layers.length, source_id, user.id);
                res.json({
                    layers,
                });
                next();
...
router.get(
    ...
    (req, res, next) => {
        ...
        load_places_mapping[provider_name](req, source, query_input)
            .then((places) => {
                logger.debug('%s loaded `%d` places in source `%s` for user `%s`',
                             req.id, places.length, source_id, user.id);
                res.json({
                    places,
                });
                next();
...
router.get(
    ...
    (req, res, next) => {
        ...
        const query_input = req.query.input;
        load_contacts_mapping[provider_name](req, source, query_input)
            .then((contacts) => {
                logger.debug('%s loaded `%d` contacts in source `%s` for user `%s`',
                             req.id, contacts.length, source_id, user.id);
                res.json({
                    contacts,
                });
                next();
