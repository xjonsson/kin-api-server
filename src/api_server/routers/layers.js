...
router.patch("/:layer_id", json_parser, (req, res, next) => {
...
router.get("/:layer_id/events", (req, res, next) => {
    const layer_id = req.params.layer_id;
    const user = req.user;
    const [source_id] = split_merged_id(layer_id); // eslint-disable-line array-bracket-spacing
    const source = user.get_source(source_id);
    ...
    const load_events = load_events_mapping[provider_name];
    load_events(req, source, layer_id)
        .then(events_source => {
            logger.debug(
                "%s loaded `%d` events in layer `%s` for user `%s`",
                req.id,
                _.get(events_source, "events.length", 0),
                layer_id,
                user.id
            );
...
router.post("/:layer_id/events", json_parser, (req, res, next) => {
    const layer_id = req.params.layer_id;
    const user = req.user;
    const [source_id] = split_merged_id(layer_id); // eslint-disable-line array-bracket-spacing
    const source = user.get_source(source_id);
    ...
    const create_event = create_event_mapping[provider_name];
    create_event(req, source, layer_id, req.body, notify_attendees)
        .then(event => {
            logger.debug("%s created event `%s` for user `%s`", req.id, event.id, user.id);
            res.json({
                event
            });
            next();
